...
def make_manager(connector, num_extra_kv_tokens=0, is_draft=False):
    """A cache manager carrying only the fields the prefix path reads.
...
class TestAskTiming:
    ...
    This is the whole of A0: the query is downstream of every drop, which is
    what guarantees *asked => scheduled*.
    """
    ...
    def test_an_excluded_request_is_still_reported_once(self):
        """The allocation is reported for every first-chunk sequence added,
    ...
    def test_a_completed_async_load_is_not_asked_again(self):
        """``should_add_sequence`` is the re-entry gate.
...
class TestChunkedBeyondTheChunk:
    ...
    by the offer plus the compute the scheduler budgeted, bounded by one
    chunk.
    """
    ...
    def test_a_non_final_chunk_end_stays_block_aligned(self):
        """Otherwise the next chunk fragments the cache."""
...
class TestExclusions:
    @pytest.mark.parametrize(
        ...
        [
            ("is_dummy", True),
            ("is_generation_only_request", True),
            ("is_disagg_generation_init_state", True),
        ],
    )
    def test_request_kinds_that_are_never_asked(self, attribute, excluded):
        connector = FakeConnectorManager(num_matched=64)
        manager = make_manager(connector)
        req = FakeRequest()
        setattr(req, attribute, excluded)
...
class TestAsyncLoad:
    ...
    def test_the_async_hold_survives_an_offer_honoured_for_nothing(self):
        """Zero honoured still parks the request, and the pass runs nothing.

        This is the one pair ``query_num_new_matched_tokens`` rejects on the way
        in, and it is deliberate on the way out. The connector started the
        transfer as soon as it answered asynchronously -- a parked request never
        reaches ``start_load_kv``, so that is the only place it can start -- and
        nothing tells it the offer was declined. Committing ``False`` here would
        leave the request in the batch and let prefill write the pages the
        transfer is landing in.
        """
        connector = FakeConnectorManager(num_matched=PROMPT_LEN, load_async=True)
        manager = make_manager(connector)
        req = FakeRequest()

        served = serve(
            manager, req, FakeKvCache(committed=0, capacity=TOKENS_PER_BLOCK, grow_ok=False)
        )

        assert not served
        assert connector.commits == [(0, 0, True)]
        assert req.context_current_position == 0, (
            "nothing was honoured, so the forward pass must still compute the "
            "whole prompt once the load completes"
        )
...
class TestReEntryAfterAServe:
    ...
    Two paths reach it. An asynchronous load parks the request out of the batch,
    and the scheduler runs ``prepare_context`` on it again when it returns; a
    batch dropped after ``prepare_resources`` brings a synchronous serve back
    the same way.
    """
    ...
    def test_the_chunk_still_spans_to_the_end_of_the_prompt(self):
        """The position and the chunk have to move together.

        ``prepare_context`` re-derives both. Leaving the chunk narrowed by the
        offer makes the pair describe two different ranges: ``position + chunk``
        stops reaching ``prompt_len``, so a non-chunked request silently becomes
        a chunked one and the forward pass is handed inconsistent metadata.
        """
    ...
    def test_the_served_position_survives_re_entry(self):
        """The cache commits the local match alone, so settling on it would put
        the request back below the prefix the connector already wrote."""
        req, served_position = self._serve_then_re_enter()

        assert req.context_current_position == served_position
        assert req.is_first_context_chunk

    def test_a_dead_allocation_drops_the_floor(self):
        """A replay recomputes from its own match: the served position vouches
        for pages that died with the allocation."""
        connector = FakeConnectorManager(num_matched=64, load_async=True)
        manager = make_manager(connector)
        req = FakeRequest()
        manager.kv_cache_map[req.py_request_id] = FakeKvCache(committed=0, capacity=PROMPT_LEN)
        ...
        req.py_connector_allocation_reported = False  # what free_resources does
        req.py_connector_served_position = 0  # ... and this
