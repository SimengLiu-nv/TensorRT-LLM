...
#
# Tests for the KV cache layout description handed to a KV connector
# (``connectors/kv_cache_layout.py``).
#
# The layout replaces the single-pool-tensor registration for a cache the tensor
# cannot express: one slot address space per pool, and one page-index space per
# layer group.
#
# Region-arithmetic and lifecycle-configuration tests need no GPU. The manager
# layout tests construct a real cache manager and allocate device memory pools.
...
@pytest.mark.parametrize(
    ("window_size", "num_sink_tokens", "expected_sink_blocks", "expected_stale_range"),
    [
        (None, None, 0, (0, 0)),
        (None, 0, 0, (0, 0)),
        (16, None, 0, (0, 6)),
...
def test_attention_life_cycle_optional_configuration(
    window_size: int | None,
    num_sink_tokens: int | None,
    expected_sink_blocks: int,
    expected_stale_range: tuple[int, int],
) -> None:
    """Both runtime backends preserve absent windows and optional sink tokens."""
    life_cycle = AttnLifeCycle.make(window_size, num_sink_tokens, 8)
    assert life_cycle.window_size == window_size
    assert life_cycle.num_sink_blocks == expected_sink_blocks
    stale = life_cycle.get_stale_range(64, 8)
    assert (stale.beg, stale.end) == expected_stale_range
    assert life_cycle == AttnLifeCycle(window_size, expected_sink_blocks)
...
class TestKvCacheRegionArithmetic(unittest.TestCase):
    ...
    def _pool_layout(
        ...
        Buffers are laid out layer-major and ascending, the way the storage
    ...
    def test_single_pool_view_ignores_the_layer_grouping_order(self):
        ...
        # not creep back in -- it once did, and sent every connector run
    ...
    def test_single_pool_view_declines_what_it_cannot_describe(self):
        ...
        # Buffer order is deliberately not a decline case. A coalesced buffer
        # is laid out layer-major and ascending when the storage config walks
        # `config.layers`, so dimension 1 is layer-indexed and there is nothing
        # here to re-derive.
...
# The classes below call `torch.cuda.init()`. `unittest/_torch/executor` is
# listed in the GPU-less l0_cpu stage, so the requirement is declared rather
# than left to fail there. It is per class, not module-level: the arithmetic
# and filter tests above are pure Python and must keep running.
@unittest.skipUnless(torch.cuda.is_available(), "allocates real device memory")
...
class TestBuildKvCacheLayoutV2(unittest.TestCase):
    """The builder against a real cache manager."""
...
class TestBuildKvCacheLayoutV2Vswa(unittest.TestCase):
    """The builder against a real cache manager with more than one window.
    ...
    #: Two distinct windows over 4 layers: the cyclic assignment gives
    #: layers 0, 2 the sliding window and layers 1, 3 full attention (``max_attention_window`` entries equal to
