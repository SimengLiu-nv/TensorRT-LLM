...
def test_connector_scheduler_output_chunked_context(enforce_single_worker,
    ...
    for i, call in enumerate(scheduler.build_connector_meta.call_args_list):
        ...
        if i == 0:
            ...
            # upfront; the blocks are whatever has been allocated so far.
...
# every layer the same window -- one layer group with a live window -- while
...
def test_connector_uniform_sliding_window(enforce_single_worker,
    ...
    # A single layer group keeps the flat list meaningful.
...
@pytest.mark.parametrize("use_kv_cache_manager_v2", [False, True],
                         ids=["kv_cache_manager_v1", "kv_cache_manager_v2"],
                         indirect=True)
def test_connector_spec_dec_is_rejected_on_v2_only(enforce_single_worker,
                                                   model_with_connector,
                                                   use_kv_cache_manager_v2):
    """Speculative decoding with a connector: V2 refuses, V1 still constructs.

    A rejected draft token shrinks the request's page list, and the slot the
    tail block held is recycled to whichever request allocates next.
    `RequestData` carries only the pages appended since the last report, so the
    connector is never told the block moved and keeps addressing a slot another
    request now owns.

    Asserting that V1 still constructs is the load-bearing half. The same
    shrink reaches V1, so the combination is no safer there -- but a guard
    written against the config rather than the manager would reject both, and
    this is what says which one is gated.
    """
    model_fn, _, _ = model_with_connector
    llm_kwargs = dict(speculative_config=NGramDecodingConfig(
        max_draft_len=4,
...
@pytest.mark.parametrize("use_kv_cache_manager_v2", [False, True],
                         ids=["kv_cache_manager_v1", "kv_cache_manager_v2"],
                         indirect=True)
def test_connector_max_utilization_is_rejected_on_v1_only(
        enforce_single_worker, model_with_connector, use_kv_cache_manager_v2):
    """A policy other than `GUARANTEED_NO_EVICT`: V1 refuses, V2 accepts.

    A policy that destroys and replays a live request brings it back on
    different pages. `KVCacheManagerV2` drops the connector's per-request block
    delta when the allocation dies, so the replay reports from scratch; V1
    keeps the delta and would measure it against pages that are gone.

    V2 accepting is the load-bearing half. `KVCacheV2Scheduler` runs
    `MAX_UTILIZATION` whatever policy is configured, so a guard that still
    refused here would reject the policy V2 actually uses while admitting the
    one it discards.
    """
    model_fn, _, _ = model_with_connector
    llm_kwargs = dict(scheduler_config=SchedulerConfig(
    ...
    if use_kv_cache_manager_v2:
        ...
        with pytest.raises(NotImplementedError, match="GUARANTEED_NO_EVICT"):
            model_fn(**llm_kwargs)
...
def test_connector_e2e_persistent_cache(enforce_single_worker: None,
    ...
    try:
        ...
        # Observed: the two runs agreed on 31 of 32 tokens and split on
    ...
    try:
        ...
        def build():
            return vswa.build_llm(
                ...
                # wrong KV; that is a separate defect, tracked in the backlog,
                # not something for this test to carry.
