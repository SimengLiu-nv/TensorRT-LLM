...
std::string formatWindowMetadataForLog(std::map<SizeType32, WindowSizeMetadata> const& metadataByWindow)
{
    ...
    for (auto const& [windowSize, metadata] : metadataByWindow)
    {
        ...
        first = false;
        os << windowSize << ":" << (metadata.isSWA ? "swa" : "full") << ":max=" << metadata.maxTokenNum
           << ":tmp=" << metadata.temporaryAttentionWindow;
    }
    return os.str();
}

bool shouldLogReuseSummaryOnce(char const* scope, tensorrt_llm::batch_manager::LlmRequest::RequestIdType requestId,
    SizeType32 windowSize, void const* owner = nullptr)
{
    static std::mutex sMutex;
    static auto* sLoggedKeys = new std::unordered_set<std::string>();

    auto const key = std::string(scope) + ":" + std::to_string(reinterpret_cast<std::uintptr_t>(owner)) + ":"
        + std::to_string(requestId) + ":" + std::to_string(windowSize);
...
void logKvCacheBlockQueryReturn(char const* functionName, tensorrt_llm::batch_manager::LlmRequest const& req,
    SizeType32 windowSize, SizeType32 returnBlocks, char const* returnPath, void const* manager = nullptr,
    char const* windowSet = "") noexcept
{
    bool const logDebug = isKvCacheReuseDebugLoggingEnabled();
    bool const logSummary = isKvCacheReuseSummaryLoggingEnabled();
    if (!logDebug && !logSummary)
    {
        return;
    }
    ...
    {
        TLLM_LOG_INFO(
            "[kv-reuse-summary] %s return manager=%p windowSet=%s request=%lu window=%d returnBlocks=%d path=%s "
            "estimatedReusableTokens=%d contextInit=%s firstContextChunk=%s disaggGenInit=%s generationInProgress=%s "
            "contextFinished=%s promptLen=%d maxNewTokens=%d contextCurrentPosition=%d",
            functionName, manager, windowSet, req.mRequestId, windowSize, returnBlocks, returnPath,
            req.getEstimatedReusableTokens(), logBool(req.isContextInitState()), logBool(req.isFirstContextChunk()),
            logBool(req.isDisaggGenerationInitState()), logBool(req.isGenerationInProgressState()),
            logBool(req.isContextFinished()), req.mPromptLen, req.mMaxNewTokens, req.getContextCurrentPosition());
    }
}
...
SizeType32 getFirstRealBlockIdxForWindow(
    WindowSizeMetadata const& metadata, SizeType32 inputLength, SizeType32 tokensPerBlock, bool swaContextReuse)
{
    // Generation-optimized SWA keeps only a bounded physical tail. Context-reuse
    // mode materializes the full prefix so future requests can reuse it.
    if (metadata.isSWA && swaContextReuse)
    {
        return 0;
    }
...
std::map<SizeType32, float> BlockManager::calculateWindowSizeToShare(
    ...
    std::optional<TempAttentionWindowInputs> const& tempAttentionWindowInputs, std::string const& optimizationTarget)
{
    TLLM_CHECK_WITH_INFO(executor::KvCacheConfig::isValidOptimizationTarget(optimizationTarget),
        "kv_cache_config.optimization_target must be one of '%s' or '%s', got '%s'",
        executor::KvCacheConfig::kContextReuseOptimizationTarget,
        executor::KvCacheConfig::kGenerationOptimizationTarget, optimizationTarget.c_str());
    ...
    for (auto const& [windowSize, _] : windowSizeToLayers)
    {
        ...
        auto const useFullInputForWindow
            = optimizationTarget == executor::KvCacheConfig::kContextReuseOptimizationTarget
            && tempAttentionWindowInputs && tempAttentionWindowInputs->maxInputLen > windowSize;
        auto const effectiveWindowSize
            = useFullInputForWindow ? tempAttentionWindowInputs->maxInputLen : windowSize + temporaryAttentionWindow;
    ...
    GenerationRequest& sequence, LlmRequest& llmRequest, ClaimResult& claimResult, bool isEnableBlockReuse)
{
    ...
    for (; bi < claimResult.numSharedContextBlocks; ++bi)
    {
        ...
        if (isRecurrentState())
        {
            ...
            {
                // When block reuse is disabled, only the last context block needs real memory
                // to store the current recurrent state. All other blocks are placeholders.
                shouldAllocate = (bi == claimResult.numContextBlocks - 1);
            }
            TLLM_LOG_DEBUG(
    ...
    SizeType32 inputLength, SizeType32 numContextBlocks, SizeType32 firstRealBlockIdx, LlmRequest& llmRequest)
{
    ...
    // Prepare block keys (needed by onboardAndAllocateBlocks for setBlockKey/setHash)
    bool const isSelfCache = mCacheType == CacheType::kSELF || mCacheType == CacheType::kSELFKONLY;
    bool const hasUniqueTokens = isSelfCache
        || (llmRequest.getEncoderUniqueTokens().has_value() && llmRequest.getEncoderUniqueTokens().value());
    ...
    // Use unified isShareLastContextBlock formula (aligned with PR #10437)
    bool const isShareLastContextBlock = mCacheType == CacheType::kCROSS || inputLength % mTokensPerBlock == 0;
    auto const beamWidth = sequence.getBeamWidth();
    result.numSharedContextBlocks
        = (beamWidth > 1 && !isShareLastContextBlock) ? numContextBlocks - 1 : numContextBlocks;
    ...
    std::vector<std::reference_wrapper<LlmRequest>> const& llmRequests, bool isEnableBlockReuse)
{
    ...
    {
        // Non-reuse: build ClaimResult with metadata only, no radix tree walk.
        // All blocks will be freshly allocated in Phase 2.
        for (size_t i = 0; i < n; ++i)
        {
...
KVCacheManager::KVCacheManager(SizeType32 numLayers, SizeType32 numKvHeads, SizeType32 sizePerHead,
    ...
    CacheType cacheType, bool enablePartialReuse, bool copyOnPartialReuse, bool enableIndexerKCache,
    SizeType32 indexerKCacheQuantBlockSize, SizeType32 indexerKCacheIndexHeadDim,
    std::optional<LinearAttentionMetadata> linearAttentionMetadata, bool swaContextReuse)
...
KVCacheManager::KVCacheManager(std::vector<SizeType32> const& numKvHeadsPerLayer, SizeType32 sizePerHead,
    ...
    : KVCacheManager(numKvHeadsPerLayer, sizePerHead, tokensPerBlock, blocksPerWindow, maxNumSequences, maxBeamWidth,
        ...
        linearAttentionMetadata, swaContextReuse)
{
}
...
KVCacheManager::KVCacheManager(std::vector<SizeType32> const& numKvHeadsPerLayer, SizeType32 sizePerHead,
    ...
    , mSwaContextReuse{swaContextReuse}
{
    ...
    TLLM_LOG_DEBUG("SWA context reuse is %s", mSwaContextReuse ? "enabled" : "disabled");
...
bool KVCacheManager::isSwaContextReuseEnabled() const
{
    return mSwaContextReuse || isSwaFullContextReuseProbeEnabled();
}
    ...
    std::optional<PrefixReuseSummary> const& cachedSummary) const
{
    ...
    auto const returnLogWindowSet
        = logKvReuseSummary ? formatWindowMetadataForLog(mBlockManager.getWindowSizesMetadata()) : std::string{};
    auto const logReturn = [&](SizeType32 returnBlocks, char const* returnPath)
    {
        logKvCacheBlockQueryReturn("KVCacheManager::getNeededBlocksOneStep", req, windowSize, returnBlocks, returnPath,
            static_cast<void const*>(this), returnLogWindowSet.c_str());
    };
    ...
    if ((req.isContextInitState() && req.isFirstContextChunk()) || req.isDisaggGenerationInitState())
    {
        ...
        auto const swaContextReuse = metadata.isSWA && isSwaContextReuseEnabled();
        ...
        auto const promptCacheLen
            = (swaContextReuse ? requestedPromptCacheLen : std::min(requestedPromptCacheLen, windowSize + chunkSize))
            + mSinkBubbleLength;
        ...
        if (mEnableBlockReuse && !isCrossKv())
        {
            ...
            auto const promptInputLen
                = (swaContextReuse || (metadata.isSWA && isSwaFullContextBlockBudgetProbeEnabled()))
            ...
            if (isKvCacheReuseSummaryLoggingEnabled()
                && shouldLogReuseSummaryOnce("getNeededBlocksOneStep", req.mRequestId, windowSize, this))
            {
                auto const windowSet = formatWindowMetadataForLog(mBlockManager.getWindowSizesMetadata());
                TLLM_LOG_INFO(
                    "[kv-reuse-summary] KVCacheManager::getNeededBlocksOneStep manager=%p windowSet=%s "
                    "request=%lu window=%d isSWA=%s "
        ...
        logReturn(numRequiredBlocks, "context-init");
    ...
    if (req.isGenerationInProgressState())
    {
        if (isCrossKv())
        {
            ...
            logReturn(kReturnBlocks, "generation-cross-kv");
        ...
        {
            constexpr SizeType32 kReturnBlocks = 0;
            logReturn(kReturnBlocks, "generation-window-limit");
            return kReturnBlocks;
        }
        ...
        logReturn(numRequiredBlocks, "generation");
        return numRequiredBlocks;
    }

    constexpr SizeType32 kReturnBlocks = 0;
    logReturn(kReturnBlocks, "default");
    return kReturnBlocks;
}
    ...
    LlmRequest const& req, SizeType32 windowSize, std::optional<PrefixReuseSummary> const& cachedSummary) const
{
    ...
    auto const logReturn = [&](SizeType32 returnBlocks, char const* returnPath)
    {
        logKvCacheBlockQueryReturn("KVCacheManager::getRemainingBlocksToCompletion", req, windowSize, returnBlocks,
            returnPath, static_cast<void const*>(this), returnLogWindowSet.c_str());
    };
    ...
    if (isCrossKv())
    {
        if (req.isContextInitState() && req.getContextCurrentPosition() == 0)
        {
            ...
            logReturn(remainingBlocks, "cross-kv-initial-context");
            return remainingBlocks;
        }

        constexpr SizeType32 kReturnBlocks = 0;
        logReturn(kReturnBlocks, "cross-kv-after-initial-context");
    ...
    if (windowSize == LinearAttentionMetadata::kRecurrentStates)
    {
        ...
        {
            constexpr SizeType32 kReturnBlocks = 0;
            logReturn(kReturnBlocks, "recurrent-generation");
        ...
        else if (!req.isContextFinished())
        {
            ...
            {
                constexpr SizeType32 kReturnBlocks = 0;
                logReturn(kReturnBlocks, "recurrent-existing-sequence");
                return kReturnBlocks;
            }
            if (mEnableBlockReuse)
            {
                ...
                logReturn(remainingBlocks, "recurrent-context-reuse");
                return remainingBlocks;
            }
            constexpr SizeType32 kReturnBlocks = 1;
            logReturn(kReturnBlocks, "recurrent-context-no-reuse");
            return kReturnBlocks;
        }
    }
    ...
    auto const contextTokenLimit
        = swaContextReuse ? req.mPromptLen : std::min(req.mPromptLen, windowSize + temporaryAttentionWindow);
    SizeType32 const numContextBlocks = (contextTokenLimit + mSinkBubbleLength) / getTokensPerBlock();

    auto const totalTokenLimit = swaContextReuse
    ...
    if (isKvCacheReuseSummaryLoggingEnabled()
        && shouldLogReuseSummaryOnce("getRemainingBlocksToCompletion", req.mRequestId, windowSize, this))
    {
        auto const windowSet = formatWindowMetadataForLog(mBlockManager.getWindowSizesMetadata());
        TLLM_LOG_INFO(
            "[kv-reuse-summary] KVCacheManager::getRemainingBlocksToCompletion manager=%p windowSet=%s "
            "request=%lu window=%d isSWA=%s "
    ...
    logReturn(remainingBlocks, "normal");
    return remainingBlocks;
}
    ...
    std::vector<std::reference_wrapper<LlmRequest>> const& llmRequests)
{
    ...
    for (auto const windowSize : orderedWindowSizes)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            firstRealBlockIdxVec[i]
                = getFirstRealBlockIdxForWindow(metadata, inputLength, getTokensPerBlock(), swaContextReuse);
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            if (logKvReuseSummary
                && shouldLogReuseSummaryOnce(
                    "addSequenceBatchWindow", llmRequests[i].get().mRequestId, windowSize, this))
            {
                auto const promptLen = llmRequests[i].get().getPromptLen();
                auto const physicalContextBlocks = numContextBlocksVec[i] - firstRealBlockIdxVec[i];
                auto const currentMinNonSwaPrepopulatedLen
                ...
                auto const windowSet = formatWindowMetadataForLog(mBlockManager.getWindowSizesMetadata());
                TLLM_LOG_INFO(
                    "[kv-reuse-summary] KVCacheManager::addSequenceBatch window-result manager=%p windowSet=%s "
                    "request=%lu window=%d isSWA=%s promptLen=%d inputLength=%d effectiveInputLength=%d "
                    "temporaryAttentionWindow=%d maxTokenNum=%d tokensPerBlock=%d numContextBlocks=%d "
                    "firstRealBlockIdx=%d physicalContextBlocks=%d windowPrepopulatedLen=%d "
                    "minAllPrepopulatedLen=%d hasNonSwaWindow=%s minNonSwaPrepopulatedLen=%d hasSwaWindow=%s "
                    "minSwaPrepopulatedLen=%d allocTotalDelta=%d allocNewDelta=%d reusedDelta=%d missedDelta=%d "
                    "primaryUsedBlocks=%d primaryFreeBlocks=%d primaryMaxBlocks=%d primaryUsedPct=%.2f",
                    static_cast<void const*>(this), windowSet.c_str(), llmRequests[i].get().mRequestId, windowSize,
                    logBool(metadata.isSWA), promptLen, std::get<1>(requestInfos[i]), inputLengths[i],
                    temporaryAttentionWindow, maxTokenNum, getTokensPerBlock(), numContextBlocksVec[i],
                    firstRealBlockIdxVec[i], physicalContextBlocks, stats.prepopulatedLen, minPrepopulatedLen[i],
                    logBool(hasNonSwaWindow[i]), currentMinNonSwaPrepopulatedLen, logBool(hasSwaWindow[i]),
                    currentMinSwaPrepopulatedLen, stats.allocTotalDelta, stats.allocNewDelta, stats.reusedDelta,
                    stats.missedDelta, primaryUsedBlocks, primaryFreeBlocks, primaryMaxBlocks,
                    primaryMaxBlocks > 0
                        ? 100.0 * static_cast<double>(primaryUsedBlocks) / static_cast<double>(primaryMaxBlocks)
                        : 0.0);
            }
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        if (mEnableBlockReuse)
        {
            ...
            if (logKvReuseSummary
                && shouldLogReuseSummaryOnce(
                    "addSequenceBatchSelect", llmRequest.mRequestId, static_cast<SizeType32>(-1), this))
            {
                auto const smallestSwaWindow = getSmallestSwaWindow(mBlockManager.getWindowSizesMetadata());
                auto const fullAttentionPrepopulatedLen
                    = hasNonSwaWindow[i] ? minNonSwaPrepopulatedLen[i] : static_cast<SizeType32>(-1);
                ...
                auto const windowSet = formatWindowMetadataForLog(mBlockManager.getWindowSizesMetadata());
                TLLM_LOG_INFO(
                    "[kv-reuse-summary] KVCacheManager::addSequenceBatch select manager=%p windowSet=%s "
                    "request=%lu promptLen=%d "
                    "tokensPerBlock=%d hasNonSwaWindow=%s hasSwaWindow=%s fullAttentionPrepopulatedLen=%d "
                    "swaPrepopulatedLen=%d minAllPrepopulatedLen=%d reusablePrepopulatedLen=%d "
                    "swaSafePrepopulatedLen=%d smallestSwaWindow=%d selectedPrepopulatedLen=%d "
                    "uncachedSuffix=%d cappedPrepopulatedLen=%d cappedUncachedSuffix=%d capWouldApply=%s "
                    "swaWouldLowerTokens=%d totalAllocTotalDelta=%d totalAllocNewDelta=%d totalReusedDelta=%d "
                    "totalMissedDelta=%d",
                    static_cast<void const*>(this), windowSet.c_str(), llmRequest.mRequestId, llmRequest.getPromptLen(),
                    getTokensPerBlock(), logBool(hasNonSwaWindow[i]), logBool(hasSwaWindow[i]),
                    fullAttentionPrepopulatedLen, swaPrepopulatedLen, minPrepopulatedLen[i], reusablePrepopulatedLen,
                    swaSafePrepopulatedLen, smallestSwaWindow.value_or(static_cast<SizeType32>(-1)),
        ...
        llmRequest.updateAllocTotalBlocksPerRequest(totalAllocTotalDelta[i]);
        llmRequest.updateAllocNewBlocksPerRequest(totalAllocNewDelta[i]);
    ...
    std::optional<LinearAttentionMetadata> const& linearAttentionMetadata)
{
    ...
    {
        windowSizeToShare = BlockManager::calculateWindowSizeToShare(windowSizeToLayers,
            cacheSizeBytesPerTokenPerWindow, tempAttentionWindowInputs, config.getOptimizationTarget());
        TLLM_LOG_DEBUG("Using derived proportional allocation of memory to each effective window size for target %s.",
            config.getOptimizationTarget().c_str());
    }
