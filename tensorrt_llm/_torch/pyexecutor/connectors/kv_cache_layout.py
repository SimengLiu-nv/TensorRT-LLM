...
"""KV cache layout description handed to a KV connector.

The cache allocates *pool groups*, each holding ``num_slots`` slots. A slot holds
the coalesced buffers of one *layer group* (a life cycle), and a buffer is keyed
by ``BufferId(layer_id, role)``. A connector therefore cannot be handed a single
...
that repeat per page slot. The addressing contract is taken verbatim from
``AggregatedPageDesc``::
...
class KvCacheLayout:
    ...
    def as_single_pool_tensor(self) -> Optional[torch.Tensor]:
        ...
        Dimension 1 is the region's buffers in memory order, laid out layer-major
        and ascending by construction: the storage config walks ``config.layers``
        in order and appends each layer's buffers to the coalesced buffer, then
        assigns offsets by walking that list. So the dimension is indexed by
        layer directly. The order is not re-derived here; if that construction
        ever changes, this is what changes with it.
        """
...
def _global_layer_ids(manager: "KVCacheManagerV2", local_layer_ids) -> List[int]:
    """Map internal layer ids to global model layer indices.
...
def build_kv_cache_layout_v2(manager: "KVCacheManagerV2") -> KvCacheLayout:
    """Describe the cache's page regions for a KV connector.
